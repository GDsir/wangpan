...
import com.hjk.wangpan.utils.RedisUtils;
...
@RequestMapping(value = "/api/passage")         //使链接还有一个 /api/
public class LikePostController {
    ...
    public Map<String, Object> giveLike(@PathVariable int id, @RequestParam("token") String token) {
        //获取请求时的token
        ...
        if (claims != null) {
            try {
                ...
                redisUtils.set("like:user_" + id, openid);
    ...
    @GetMapping(value = "/cancellike/{id}")
    @ApiOperation(value = "用户取消点赞接口", notes = "用户取消点赞接口")
    public Map<String, Object> giveCancelLike(@PathVariable int id, @RequestParam("token") String token) {
        //获取请求时的token
        JwtUtils jwt = JwtUtils.getInstance();
        Claims claims = jwt.check(token);
        if (claims != null) {
